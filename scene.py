...
class Calculus(Scene):
    ...
    def caption(self, text, size=26, color=GREY_B, edge=DOWN, buff=0.6):
    ...
    def beat_intro(self):
        ...
        cap = Text("a moving object:   position  s(t) = t²", font_size=28, color=BLUE)
        ...
        self.add(dot)
        self.play(t_tr.animate.set_value(4.2), run_time=6, rate_func=linear)
        self.wait(1.0)

        # average speed over the whole trip = secant slope
        sec = Line(axes.c2p(0, 0), axes.c2p(4.2, 4.2 ** 2), color=YELLOW, stroke_width=3)
        avg = Text("average speed  =  17.6 / 4.2  ≈  4.2", font_size=26, color=YELLOW)
        avg.next_to(sec, UP, buff=0.3)
        self.play(ShowCreation(sec), Write(avg))
        self.wait(2.5)

        self.play(FadeOut(sec), FadeOut(avg))
        q = Text("But how fast is it going at exactly one instant?", font_size=30, color=YELLOW)
    ...
    def beat_limit(self):
        ...
        cap = Text("What is the slope of the curve at a single point?", font_size=26)
        ...
        explain = Text("a secant line through two points gives an average slope",
        ...
        for h in hs:
            Q = Dot(axes.c2p(X0 + h, f(X0 + h)), color=GREEN)
            sec = Line(axes.c2p(X0, f(X0)), axes.c2p(X0 + h, f(X0 + h)),
                       color=YELLOW, stroke_width=3)
            slope = (f(X0 + h) - f(X0)) / h
            new = Tex(f"slope = {slope:.2f}", font_size=32, color=YELLOW)
            new.to_corner(UR)
            self.play(ShowCreation(sec), FadeIn(Q), Transform(slope_tex, new), run_time=1.2)
            self.wait(1.0)
            self.play(FadeOut(sec), FadeOut(Q), run_time=0.4)
        ...
        note = Text("as the second point slides closer, the secant becomes the tangent",
                    font_size=22, color=GREY_B)
        ...
        self.play(Write(lim))
        self.wait(2.5)
        never = Text("we never divide by zero — we ask what the value approaches",
                     font_size=22, color=GREY_B)
        never.next_to(lim, DOWN, buff=0.2)
        self.play(Write(never))
        self.wait(3.5)
        self.play(FadeOut(axes), FadeOut(graph), FadeOut(P), FadeOut(P_label),
                  FadeOut(cap), FadeOut(slope_tex), FadeOut(tan), FadeOut(note),
                  FadeOut(lim), FadeOut(never))
    ...
    def beat_derivative(self):
        ...
        self.wait(2.5)
        geom = Text("the derivative = slope of the tangent = instantaneous rate of change",
                    font_size=22, color=GREY_B)
        ...
        top_cap = Text("the function f", font_size=20, color=BLUE)
        top_cap.next_to(axes_f, LEFT, buff=0.3)
        bot_cap = Text("plot the slope at each point -> the derivative f '", font_size=20, color=YELLOW)
        bot_cap.next_to(axes_fp, LEFT, buff=0.3)

        self.play(ShowCreation(axes_f), ShowCreation(g_f), Write(lab_f), Write(top_cap))
        self.wait(1.5)
        self.play(ShowCreation(axes_fp), Write(lab_fp), Write(bot_cap))
        self.wait(1.5)
        ...
        self.add(dot_f, tan)
        self.play(x_tr.animate.set_value(2.3), run_time=7)
        self.wait(1.0)

        # reset, then trace f' on the lower axes
        self.play(x_tr.animate.set_value(-2.3), run_time=1.5)
        ...
        self.play(x_tr.animate.set_value(2.3), run_time=9)
        self.play(Transform(lab_fp, new_lab))
        self.wait(2.0)

        # insight: where f' = 0, f is flat (a minimum here)
        flat_cap = Text("where  f '(x) = 0  the tangent is flat — here, the minimum of f",
                        font_size=20, color=GREY_B)
        flat_cap.to_edge(UP, buff=0.3)
        self.play(Write(flat_cap))
        self.play(x_tr.animate.set_value(0.0), run_time=2)
        self.wait(3.0)

        note = Tex(r"\frac{dy}{dx} = f'(x) = 2x", font_size=32)
        note.to_edge(UP, buff=0.4)
        self.play(FadeOut(flat_cap), Write(note))
        self.wait(3.0)
        ...
        self.play(FadeOut(axes_f), FadeOut(axes_fp), FadeOut(g_f), FadeOut(lab_f),
                  ...
                  FadeOut(trace), FadeOut(note), FadeOut(top_cap), FadeOut(bot_cap))
    ...
    def beat_power_rule(self):
        ...
        for ln, ex in zip(lines, explain):
            self.play(Write(ln), run_time=1.2)
            side = Text(ex, font_size=20, color=GREY_B)
            side.next_to(ln, RIGHT, buff=0.6)
        ...
        rule.to_edge(DOWN, buff=1.8)
        box = SurroundingRectangle(rule, color=GOLD, buff=0.2)
        gen_cap = Text("the power rule — true for every whole-number power", font_size=22, color=GREY_B)
        gen_cap.next_to(rule, UP, buff=0.4)
        self.play(Write(rule), ShowCreation(box), Write(gen_cap))
        self.wait(3.0)
        pat = Tex(r"x^1 \to 1 \quad x^2 \to 2x \quad x^3 \to 3x^2 \quad x^4 \to 4x^3", font_size=30)
        pat.next_to(rule, DOWN, buff=0.5)
        self.play(Write(pat))
        self.wait(4.0)
        self.play(FadeOut(title), FadeOut(rule), FadeOut(box), FadeOut(gen_cap), FadeOut(pat))
    ...
    def beat_integral(self):
        ...
        cap = Text("how much area lies under  f(x) = x²  from 0 to 3 ?", font_size=24, color=BLUE)
        cap.to_edge(UP, buff=0.5)
        self.play(ShowCreation(axes), ShowCreation(graph), Write(cap))
        self.wait(2.5)

        how = Text("approximate it with rectangles", font_size=22, color=GREY_B)
        how.to_edge(DOWN, buff=0.5)
        self.play(Write(how))
        self.wait(2.0)
        ...
        for n in [4, 8, 16, 32, 64]:
            ...
            lab.to_corner(UR)
            if cur is None:
                self.play(LaggedStartMap(FadeIn, rects, lag_ratio=0.04), Write(lab), run_time=2)
            else:
                self.play(ReplacementTransform(cur, rects),
                          ReplacementTransform(cur_lab, lab), run_time=1.5)
            cur, cur_lab = rects, lab
            self.wait(2.0)

        self.play(FadeOut(how))
        conv = Text("the sums home in on a single number:  9", font_size=24, color=GREEN)
        conv.to_edge(DOWN, buff=0.5)
        ...
        lim_tex = Tex(
            ...
            font_size=30)
        lim_tex.to_edge(DOWN, buff=1.3)
        self.play(FadeOut(conv), Write(lim_tex))
        self.wait(3.0)
        snote = Text("the  ∫  sign is a stretched  S  for  'sum'", font_size=22, color=GREY_B)
    ...
    def beat_ftc(self):
        ...
        axes = Axes(x_range=(0, 3.6, 1), y_range=(0, 10, 2), width=7.5, height=4.6)
        ...
        Flab.to_corner(UR)
        Fcap = Text("F(x) = the area accumulated so far", font_size=22, color=GREY_B)
        Fcap.next_to(Flab, DOWN, buff=0.3)
        self.play(FadeIn(area), FadeIn(vline), Write(Flab), Write(Fcap))
        self.wait(2.0)
        self.play(x_tr.animate.set_value(3.0), run_time=9, rate_func=linear)

        fact = Tex(r"F(x) = \frac{x^3}{3}, \quad F(3) = 9", font_size=32)
        fact.next_to(Fcap, DOWN, buff=0.3)
        self.play(Write(fact))
        self.wait(3.5)

        # the sliver: area grows by ~ f(x) * dx
        xu = axes.x_axis.get_unit_size()
        yu = axes.y_axis.get_unit_size()
        dx = 0.3
        sliver = Rectangle(width=dx * xu, height=f(3.0) * yu,
                           fill_color=GREEN, fill_opacity=0.6,
                           stroke_color=GREEN, stroke_width=2)
        sliver.move_to(axes.c2p(3.0 + dx / 2, f(3.0) / 2))
        sliv_cap = Text("a small step adds area  ~  f(x) · dx", font_size=22, color=GREEN)
        sliv_cap.to_corner(UR, buff=1.6)
        self.play(FadeIn(sliver), Write(sliv_cap))
        self.wait(3.5)

        # freeze updaters, then reveal the rate
        area.clear_updaters()
        vline.clear_updaters()
        self.play(FadeOut(sliver), FadeOut(sliv_cap), FadeOut(Fcap), FadeOut(fact),
                  FadeOut(area), FadeOut(vline), FadeOut(Flab))

        # the insight, on a clean screen
        insight = Tex(r"\frac{dF}{dx} = f(x)", font_size=52, color=YELLOW)
        ...
        crown = Text("derivatives and integrals are inverses — two sides of one coin",
                     font_size=24, color=GREY_B)
        crown.next_to(box, UP, buff=0.25)
        self.play(Write(inv1), Write(inv2), ShowCreation(box), Write(crown))
        self.wait(5.0)
        self.play(FadeOut(title), FadeOut(axes), FadeOut(graph), FadeOut(inv),
                  FadeOut(box), FadeOut(crown))

    # ---------- 7. why it matters / outro ----------
    def beat_outro(self):
        head = Text("Why this matters", font_size=34, color=YELLOW)
        ...
        chain = Tex(r"s(t) \;\longrightarrow\; v(t)=s'(t) \;\longrightarrow\; a(t)=v'(t)",
                    font_size=30)
        chain.next_to(head, DOWN, buff=0.6)
        chain_cap = Text("each arrow takes a derivative:  position -> velocity -> acceleration",
                         font_size=22, color=GREY_B)
        chain_cap.next_to(chain, DOWN, buff=0.3)
        ...
        app2.next_to(ex, DOWN, buff=0.7)
        self.play(Write(app2))
        self.wait(3.0)
        app3 = Text("where  f '(x) = 0,  f has a peak or valley  ->  optimization",
                    font_size=24, color=YELLOW)
        app3.next_to(app2, DOWN, buff=0.4)
        self.play(Write(app3))
        self.wait(4.0)
        self.play(FadeOut(head), FadeOut(chain), FadeOut(chain_cap), FadeOut(ex),
                  FadeOut(app2), FadeOut(app3))

        recap = VGroup(
            Text("Limits let us speak of the instant.", font_size=32),
            Text("Derivatives measure change at an instant.", font_size=32),
            Text("Integrals accumulate change.", font_size=32),
        ).arrange(DOWN, buff=0.6)
        self.play(LaggedStart(*[Write(t) for t in recap], lag_ratio=1.0, run_time=5))
        self.wait(3.0)
        close = Text("Calculus — the language of everything that changes.",
                     font_size=34, color=YELLOW)
        close.next_to(recap, DOWN, buff=0.7)
